...
export default class TransitionItemsView extends React.Component<
  ...
  getDirectionForRoute = (name: string, route: string): RouteDirection => {
    ...
    if (!fromRoute) { return RouteDirection.to; } // First screne, always direction to
    if (route === fromRoute) {
  ...
  componentDidUpdate() {
    ...
    let measureAndUpdateFunc = async () => {
      ...
      if (!sharedElements.find(p => !p.fromItem.metrics || !p.toItem.metrics)
        ...
        if (this.compareSharedElements(this.state.sharedElements, sharedElements)
          && this.compareTransitionElements(this.state.transitionElements, transitionElements)) { return; }

        console.log('SET STATE UPDATE');
  ...
  compareTransitionElements = (e1:Array, e2:Array): boolean => {
  ...
  compareSharedElements = (e1: Array, e2: Array): boolean => {
    if (!e1 && !e2) return true;
    if ((!e1 && e2) || (e1 && !e2)) return false;
    if (e1.length !== e2.length) return false;
    for (let i = 0; i < e1.length; i++) {
