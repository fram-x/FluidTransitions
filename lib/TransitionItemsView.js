...
export default class TransitionItemsView extends React.Component<
  ...
  getRoutes = () => [this.state.fromRoute, this.state.toRoute].filter(r => r !== null)
  ...
  getIsPartOfSharedTransition = (name: string, route: string) => {
    ...
    if (sharedElements.find(pair => (pair.fromItem.name === item.name && pair.fromItem.route === item.route)
