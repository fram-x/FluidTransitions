...
type TransitionProps = {
  ...
  children: Array<any>,
  zIndex: Number,
  transitionInSharedLayer: ?boolean,
}
...
class Transition extends React.Component<TransitionProps> {
  ...
  componentWillMount() {
    ...
    const { shared, appear, disappear, delay, zIndex, anchor,
      animated, transitionInSharedLayer = false } = this.props;
    if (register) {
      this._route = route;
  ...
  getTransitionStyle() {
    const { transitionInSharedLayer = false } = this.props;
    if (transitionInSharedLayer) return {};
  ...
  getVisibilityStyle() {
    ...
    if (isPartOfSharedTransition
      || (isPartOfTransition && transitionInSharedLayer)
      || isAnchored) {
